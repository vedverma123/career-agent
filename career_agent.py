...
CRITICAL MONTHLY INSTRUCTION:
- If include_monthly is FALSE → set "monthly": null in your response
- If include_monthly is TRUE → you MUST populate the monthly field like this:
  "monthly": {{
    "linkedin_post": {{
      "angle": "one line describing the post angle",
      "draft": "full post text 150-200 words",
      "hashtags": ["#tag1", "#tag2", "#tag3", "#tag4"]
    }}
  }}
- TODAY include_monthly is: {include_monthly}
- Do NOT return null for monthly if include_monthly is true — this is mandatory
...
def build_email_html(data: dict, include_monthly: bool) -> str:
    ...
    # ── Monthly section
    monthly_section = ""
    if include_monthly and monthly and isinstance(monthly, dict):
...
def send_email(html: str, data: dict, include_monthly: bool) -> None:
    ...
    monthly_tag = " · 📝 LinkedIn Draft Ready" if (include_monthly and data.get("monthly")) else ""
    subject = (
        f"🏢 Career Intel · {week} · "
